...
<center style="font-size:20px;"> <?php include('name.php'); ?> <br> Product Sales  Profit<br></center> 
...
<?php
  ...
  $eq=mysqli_query($con,"SELECT  distinct client_contact,client_contact FROM orders where order_type=2");


  while($eqrow=mysqli_fetch_array($eq)){
    $TotalBuyRate=$eqrow['quantity']*$eqr
...
$sql = $connect->query("SELECT count(`order_item_id`) as `total4` FROM `order_item`
...
$sql = $connect->query("SELECT sum(`quantity`) as `total3` FROM `order_item`
WHERE entry_date >= '$start_date' AND entry_date <= '$end_date' and order_item_status='1' ");
$row = $sql->fetch_assoc();
$TSQ= $row['total3'];
?><b><?php echo $TSQ ; ?></b></td>
...
$sql = $connect->query("SELECT count(`order_item_id`) as `total` FROM `order_item`
WHERE entry_date >= '$start_date' AND entry_date <= '$end_date' and order_item_status='1' ");
$row = $sql->fetch_assoc();
$TSQ= $row['total'];
...
Total Collection = 
<?php //Total Product Qty
$sql = $connect->query("SELECT sum(`pre_due`) as `total` FROM `orders`
...
<?php //Total Product Qty
$sql = $connect->query("SELECT sum(`discount`) as `total` FROM `orders`
WHERE order_date >= '$start_date' AND order_date <= '$end_date'  ");
$row = $sql->fetch_assoc();
$Dis= $row['total'];
?><b> <?php echo $Dis ; ?> ) = </b>
