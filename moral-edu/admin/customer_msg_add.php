...
<?php 
	...
	if(isset($_POST['btnsave']))
	{
		...
		if(!isset($errMSG))
		{
			$stmt = $DB_con->prepare('INSERT INTO customer_msg (admin_id,user_id,msg_name,link,msg_from_to,msg_from,msg_to,msg_status,status,entry_date) VALUES(22,:User_Id,:Msg_Name,:Link,"Company":MsgFrom,:MsgTo,1,1,:Date)');
...
<form method="post" enctype="multipart/form-data" class="form-horizontal" >
	...
	<table class="table table-bordered table-responsive">
    	...
    	<td><label class="control-label">Message To</label></td>
       	<td><select style="width:100%;" class="form-control" name="User_Id" id="CustId"/>
		<option value="#">Select User</option>
				      	<?php 
				      	$sql = "SELECT  userid,business_name FROM stuff where status=1 ";
